...
document.getElementById('loginForm').addEventListener('submit', async (e) => {
  ...
  try {
    const response = await fetch('https://sarafbrk.com:8088/api/v1/auth/authenticate', {
      method: 'POST',
      headers: {
...
document.getElementById('requestPasswordReset').addEventListener('click', async function (event) {
  ...
  try {
    const response = await fetch('https://sarafbrk.com:8088/api/v1/user/forgot-password', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
